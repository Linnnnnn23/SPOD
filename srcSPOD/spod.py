...
def spod(x,dt,save_path,weight='default',nOvlp='default',nDFT='default',window='default',
    ...
    for iFreq in range(nFreq):
        ...
        M = np.dot(np.conjugate(Q_hat_f).T * weight, Q_hat_f)/nBlks # weight is diagonal matrix
...
def spod_parser(nt, nx, window, weight, nOvlp, nDFT, method):
    ...
    if (nDFT < 4) or (nBlks < 2):
        raise ValueError('User sepcified window and nOvlp leads to wrong nDFT and nBlk.nDFT=%s,nBlks=%s'%(nDFT,nBlks))
...
def reconstruct_time_method(x,dt,f,P,Ms,fs,save_path,weight='default',method='lowRAM'):
    '''
...
def getOneTimeData(timeList,dataRecPath,save_path):
    fin = h5py.File(dataRecPath,'r')
    for i in timeList:
        with open(os.path.join(save_path,'t%s.dat'%i),'w')as fileOut:
            data = fin['data_rec'][i]
